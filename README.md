...
```
rag-local/
├── main.py                      # CLI entry point (imports from src/)
...
- Loads and ingests historical documents
- Orchestrates the query loop
...
| `Slow ingestion` | Reduce CHUNK_SIZE in .env or use smaller documents |
| `Port 7687 in use` | Change NEO4J_BOLT_PORT in docker-compose.yml |
| `ImportError from src.*` | Ensure you're in the rag-local directory |
